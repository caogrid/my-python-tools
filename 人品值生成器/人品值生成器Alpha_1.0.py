...
if last_date == current_date:
    # 如果是同一天，输出上次的随机数
    print(f"Same day: Last random number was {last_random_number}")
else:
    # 如果不是同一天，生成一个新的随机数并保存
    new_random_number = random.randint(1, 100)  # 生成1到100之间的随机数
    print(f"Different day: New random number is {new_random_number}")
    
    # 将当前日期和随机数写入文件
    with open(data_file, 'w') as file:
        file.write(f"{current_date} {new_random_number}\n")

# 注意：为了测试，你可以手动修改文件内容来模拟不同天的运行
# 但请注意，在实际使用中，不要手动修改文件内容，让程序自动处理